import "../css/styles.css";

export function Interested_In() {
  return (
    <div className="App">
      {/* Header */}
      <div className="header">
        <div className="grid-box header-sub">
          <img
            className="img"
            src="/assets/images/backArrow.png"
            alt=""
            height={10}
          />
          <img
            className="img"
            src="/assets/images/skin-expert-image.png"
            alt=""
            height={50}
          />
          <img
            className="img"
            src="/assets/images/cross-icon.png"
            alt=""
            height={10}
          />
        </div>
      </div>
      <div className="flex flex-col">
        {/* Body */}
        <div className="body align-from-left">
          <div className="whats_ur_number">
            <span className="text-4 grey-text-color">
              Whom Are You Taking It For?
            </span>
          </div>
          <div className="whats_ur_number_caption">
            <span className="text-1">
              Introduce yourself fill out the details so people know about you.
            </span>
          </div>

          <div className="takingItForList">
            <div>
              <input type="radio" name="section" id="myself" defaultChecked />
              <label htmlFor="myself" className="text-2">
                Myself
              </label>
            </div>
            <div>
              <input type="radio" name="section" id="friends" />
              <label htmlFor="friends" className="text-2">
                Friends
              </label>
            </div>
            <div>
              <input type="radio" name="section" id="family" />
              <label htmlFor="family" className="text-2">
                Family
              </label>
            </div>
          </div>

          <section id="phoneNoContinue">
            <button className="text-2">Done</button>
          </section>

          <section id="footer-section">
            <div className="flex">
              <span className="text-1">Terms of Service</span>
              <span className="text-1">Privacy Policy</span>
            </div>
          </section>
        </div>
      </div>
    </div>
  );
}
